...
Inline keyboard: two buttons per finding, callback_data encodes
`{action}:{repo}:{fingerprint_hash}` (short hash, not the full fingerprint
object — keep callback_data under Telegram's 64-byte limit).
